...
bool PREDICTOR::patternpredictor::GetPatternPrediction (UINT32 PC){
    ...
    if (tag[PCINDEX][0] == tagfn(PC)) tableID = 0;
    else if (tag[PCINDEX][1] == tagfn(PC)) tableID = 1;
    else return 1;
    
    
    int i;
    for (i=1; i<= LOCALHISTORYLENGTH - MAXPLEN; i++){
        if ((pattern_sample[PCINDEX][tableID] ^ (pattern_sample[PCINDEX][tableID] >> i)) % (1 << MAXPLEN) == 0) return ((pattern_sample[PCINDEX][tableID] >> (i-1)) % 2);
    }
...
bool PREDICTOR::patternpredictor::IsPatternPredReady (UINT32 PC){
    ...
    int i;
    for (i=1; i<= LOCALHISTORYLENGTH - MAXPLEN; i++){
        if ((pattern_sample[PCINDEX][tableID] ^ (pattern_sample[PCINDEX][tableID] >> i)) % (1 << MAXPLEN) == 0) return 1;
    }
...
void PREDICTOR::patternpredictor::UpdatePatternPredictor (UINT32 PC, bool taken){
    ...
    if (tag[PCINDEX][0] == tagfn(PC)) {
        tableID = 0;
        if (GetPatternPrediction(PC) != taken || IsPatternPredReady(PC) == 0){
    ...
    pattern_sample[PCINDEX][tableID] = ((pattern_sample[PCINDEX][tableID] << 1) + taken) % (1 << 2 * MAXPLEN);
...
void PREDICTOR::patternpredictor::PrintAll (){
    ...
    cout << "\n\nReady = ";
    prch (IsPatternPredReady(1048575));
